...
class SinaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        sub_titles = response.xpath('//ul[@class="list01"]//a/text()').extract()
        sub_urls = response.xpath('//ul[@class="list01"]//a/@href').extract()
        items = []
    ...
    def parse_second(self, response):
        ...
        for son_url in son_urls:
            if son_url.startswith(parent_url) and son_url.endswith('.shtml'):
                yield scrapy.Request(son_url, callback=self.parse_detail, meta={'item': item})
